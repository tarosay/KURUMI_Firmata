...
namespace Firmata.NET
{
    ...
    class Arduino
    {
        ...
        private volatile int[] digitalOutputData = { 0, 0, 0, 0, 0, 0, 0, 0 };
        private volatile int[] digitalInputData =  { 0, 0, 0, 0, 0, 0, 0, 0 };
        private volatile int[] analogInputData =   { 0, 0, 0, 0, 0, 0, 0, 0 };
        ...
        public void digitalWrite(int pin, int value)
        {
            ...
            message[0] = (byte)(DIGITAL_MESSAGE | portNumber);          //0x90 - 0x9F
            message[1] = (byte)(digitalOutputData[portNumber] & 0x7F);  //0-6
            message[2] = (byte)(digitalOutputData[portNumber] >> 7);    //7-13
            _serialPort.Write(message, 0, 3);
        }
            ...
            while (_serialPort.IsOpen)
            {
                if (_serialPort.BytesToRead > 0)
                {
                    lock (this)
                    {
                        ...
                        if (parsingSysex)
                        {
                            Debug.WriteLine(parsingSysex.ToString());
